...
def gather_jobs(profile: Dict) -> List[Dict]:
    ...
    for term in profile.get("search_terms", []):
        print(f"[api] Fetching listings for: {term}")
...
def init_db():
    ...
    conn.commit()
    conn.close()
    print("[db] Tables ready and schema initialized.")
...
{cv_text}"""
    ...
    print(f"[phase 1] Strategy built. Target Region: {profile['location']} (Adzuna Node: {profile['adzuna_country_code']})")
    return profile
...
def api_candidates(jobs, profile_embedding):
    ...
    candidates = []
    for job,确定_emb in zip(jobs, embeddings):
        score = cosine_similarity(profile_embedding, 确定_emb)
...
def rank_candidates(candidates: list[dict], profile: dict) -> list[dict]:
    if not candidates:
        print("[phase 4] No candidates found passing basic structural vector checks.")
        return []

    pool = sorted(candidates, key=lambda x: x.get("embed_score", 0), reverse=True)[:60]
    print(f"[phase 4] Ranking top {len(pool)} raw candidates via semantic matrix...")
...
{listing_block}"""
    ...
    try:
        ...
        print(f"[phase 4] Ranking structural processing failed ({e}). Reverting to default matrix order.")
    ...
    print(f"[phase 4] Top {len(top)} high-yield listings selected for extraction hydration.")
    return top
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if result.success and result.markdown and len(result.markdown.strip()) > 150:
                        ...
                        if attempt > 1:
                            print(f"   ✓ [RETRY SUCCESS] Bypassed script wall for {job['company']} on attempt #{attempt}")
                        break
                    else:
                    ...
                    if attempt == max_retries:
                        print(f"   [!] [PHASE 5 FAILURE] Blocked at {job['company']}. Preserving snippet summary.")
                        job["full_text"] = job.get("snippet", "")
                    else:
                        print(f"   ⚠️ [BLOCKED/SHELL] Cool-down applied for {job['company']} (Attempt #{attempt}). Retrying...")
...
def final_evaluation(jobs: list[dict], profile: dict) -> list[dict]:
    print(f"[phase 6] Final matching processing matrix active ({EXP_MODEL})...")
...
{jobs_block}"""
    ...
    try:
        ...
        print(f"[phase 6] Final generation evaluation failed: {e}")
        return []
...
def save_and_display(results: list[dict]):
    ...
    for i, job in enumerate(results, 1):
        ...
        try:
            ...
            print(f"  [save] Local tracking DB write error: {e}")
...
async def main():
    ...
    if not candidates:
        print("[!] No jobs found aligning with target candidate embedding vectors.")
        return
